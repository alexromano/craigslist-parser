...
from .utils import get_neighborhood_values
...
class Search:
    ...
    def fetch(self, filters) -> int: 
        self.request = requests.get(self.url, params = self._format_filters(filters))
    ...
    def _format_filters(self, filters: Dict) -> Dict:
        ...
        for key, value in filters.items():
            if key == "neighborhoods":
                formatted_filters['nh'] = get_neighborhood_values(value)
            else:
